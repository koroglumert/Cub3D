...
/*   Updated: 2023/09/14 13:37:31 by havyilma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	paint_cloud_and_grass(t_setting *set)
{
	...
	while (x < WIDTH)
	{
		...
		while (y < HEIGHT)
		{
			...
			//	mlx_pixel_put(set->mlx->mlx_init, set->mlx->mlx_window, x * set->mlx->bits_per_pixel  , y * set->mlx->size_line , set->map->rgb_ceiling);
			}
			...
			//	mlx_pixel_put(set->mlx->mlx_init, set->mlx->mlx_window, x * set->mlx->bits_per_pixel  , y * set->mlx->size_line, set->map->rgb_floor);
			}
			y++;
		}
		x++;
